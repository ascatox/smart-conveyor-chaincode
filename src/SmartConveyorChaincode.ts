...
export class SmartConveyorChaincode implements ChaincodeInterface {
    ...
    /* methods GET */
    /* getItemsByBayInBay() */
    /* The getItemsByBayInBay method is called to GET a subset of "Map" with all items INTO this Bay */
    /**
     * Handle custom method execution
     *
     * @param stub
     */
    public async getItemsByBayInBay(stub: Stub, bayId: string) {
        this.logger.info('************* getItemsByBayInBay *************');
        if (bayId == null || bayId == '') {
            throw new Error('getItemsByBayInBay - ERROR No  input Bay');
        }
        try {
    ...
    /* methods POST */
    /* controlItemss() */
    /* The controlItems method is called to assign bay to items suspended  */
    /**
     * Handle custom method execution
     *
     * @param stub
     */
    private async controlItems(stub: Stub) {
        this.logger.info('################ ATTENTION #########################');
        this.logger.info('########### controlItems  NOT IN FUNCTION ###########');
        this.logger.info('####################################################');
    ...
    private async doConveyorItemAssignTo(stub: Stub, item: ConveyorItem, state: ConveyorItem.State) {
        ...
        if (state == ConveyorItem.State.inBay) {
            if (item.state != ConveyorItem.State.inBelt) {
                this.logger.warn('doConveyorItemAssignTo - ITEM IN STATE NOT INBELT : ' + item.state);
            }
